...
struct PartModulesView: View
{
    ...
    private var filtered_modules: [PartModule]
    {
        if search_text.isEmpty
        {
            return base_stc.part_modules
        }
        
        return base_stc.part_modules.filter
        {
...
private struct PartModuleCard: View
{
    ...
    private func on_rename(_ new_name: String)
    {
        if !new_name.isEmpty
        {
            let unique_name = unique_name(for: new_name, in: base_stc.part_module_names.filter { $0 != module.name })
